...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.pro != true && user.todos.length == 10) {
    return response
      .status(403)
      .json({ error: "User must be in Pro plan to create more tasks!" });
  }
...
function checksTodoExists(request, response, next) {
  ...
  if (!user) {
    ...
    todoFound = user.todos.find((todo) => todo.id === id);
    if (!todoFound) {
      return response
        .status(404)
        .json({ error: "Task not found in user's list!" });
    }
  }
